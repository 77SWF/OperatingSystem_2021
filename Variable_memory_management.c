...
#define BOOL int //自定义布尔变量
#define TRUE 1
#define FALSE 0

BOOL low_is_free = FALSE;  //释放区低地址是否与空闲分区相邻
BOOL high_is_free = FALSE; //释放区高地址是否与空闲分区相邻
...
char *lmalloc(unsigned size)
{
    ...
    do
    {
        ...
        else if (now_m->size_m >= size)
        {
            ...
            //否则，若新分区大小=0，直接连接原前后指针
            else
            {
                /* free？free后该"分配出去了"的空间不能再被使用，
                后续"释放"后，怎么申请回来？
                不应free掉吧？因为在分配/释放过程中，应该总是在申请来的"模拟内存"中有效 
                */
