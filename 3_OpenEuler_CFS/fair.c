...
// 调度周期是一个动态变化的值，计算调度周期
// 参数：就绪进程数量nr_running
static u64 __sched_period(unsigned long nr_running)
{
	//就绪进程数量 超过sched_nr_latency=8时，我们无法保证调度延迟，因此转为保证调度最小粒度
	if (unlikely(nr_running > sched_nr_latency)) 
		return nr_running * sysctl_sched_min_granularity;
	else
	// 没有超过8，那么调度周期就等于调度延迟sysctl_sched_latency（6ms）
...
void reweight_task(struct task_struct *p, int prio)
{
	...
	// sched_prio_to_weight：sched_prio_to_weight[nice值]转成权重weight
...
place_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int initial)
{
	 ...
	 * little, place the new task so that it fits in the slot that “当前”阶段已经承诺给当前的任务，但是新任务的额外重量会让它们慢下来，把新任务放在最后的空位上。
...
enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	bool curr = cfs_rq->curr == se; //该进程正好占用CPU
	...
	/*
	 * When enqueuing a sched_entity, we must: 当将已调度实体排队时，必须:
	 *   - Update loads to have both entity and cfs_rq synced with now.现在更新加载使实体和cfs_rq同步
	 *   - Add its load to cfs_rq->runnable_avg 将其负载添加到cfs rq->runnable_avg
	 *   - For group_entity, update its weight to reflect the new share of 
	 *     its group cfs_rq 对于group_entity，更新其权重以反映其组cfs_rq的新共享
	 *   - Add its new weight to cfs_rq->load.weight 将其新权重添加到cfsrq->load.weight中
	 */
 ...
 * called on fork with the child task as argument from the parent's context
 在fork上调用子任务作为父任务上下文的参数
 *  - child not yet on the tasklist  子任务不在任务列表里
 *  - preemption disabled 禁止占用
 */
static void task_fork_fair(struct task_struct *p)
{
	struct cfs_rq *cfs_rq; //CFS调度器（类）的就绪队列
	struct sched_entity *se = &p->se, *curr;//p的调度实体，就绪队列现在
	struct rq *rq = this_rq();
	struct rq_flags rf;
